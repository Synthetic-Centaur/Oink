...
var populateTables = () => {
    return db.knex.schema.hasTable('users').then( (exists) => {
      if (!exists) {
        return db.knex.schema.createTable('users', (user) => {
          ...
          return db.knex.schema.hasTable('transactions').then( (exists) => {
            if (!exists) {
              return db.knex.schema.createTable('transactions', (transaction) => {
                ...
                return db.knex.schema.hasTable('categories').then( (exists) => {
                  if (!exists) {
                    return db.knex.schema.createTable('categories', (category) => {
                      category.increments('id').primary()
                      category.string('description', 255)
                    }).then((table) => {
                      console.log("Created Categories Table")
                      return db.knex.schema.hasTable('budgets').then( (exists) => {
                        if (!exists) {
                          return db.knex.schema.createTable('budgets', (budget) => {
